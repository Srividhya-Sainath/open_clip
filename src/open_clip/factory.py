...
from transformers import BertTokenizer # for the new get_tokenizer script
...
def get_tokenizer(
    ...
    if local_tokenizer_path:
        ...
        try:
            ...
            raise ValueError(f"Failed to load tokenizer from local path: {e}")
...
def create_model(
    ...
    if has_hf_hub_prefix:
        ...
        pretrained_hf = True  # override, no need to load original HF text weights
    else:
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        local_model_path = model_cfg['text_cfg'].get('local_tokenizer_path', None) ## ADDED BY ME
        if is_hf_model and local_model_path:
            print(f"Loading Hugging Face model from local path: {local_model_path}")
            model_cfg['text_cfg']['hf_model_pretrained'] = False 
            model_cfg['text_cfg']['hf_model_name'] = local_model_path
        elif is_hf_model:
            # Otherwise, load pretrained HF model if specified and not overridden by CLIP weights
            print(f"Loading Hugging Face model from Hugging Face: {model_cfg['text_cfg']['hf_model_name']}")
        ...
        if pretrained:
            ...
            print("Reached 2: ", pretrained_cfg)
            if pretrained_cfg:
